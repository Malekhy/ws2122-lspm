...
def showstats(stat1,stat2, selected_method, log_name, event_logs_path ):
    ...
    filename = str("LSPM - ") + selected_method +  str(" - ") + str(log_name) + str(time.strftime("-%H%M%S")) + str(".png")
...
def run(request):
    ...
    if request.method == 'POST':
        ...
        if (case_id == activity_name == timestamp_time):
        ...
        Slog_attributes['case_id'] = case_id
        Slog_attributes['activity_name'] = activity_name
        Slog_attributes['timestamp_time'] = timestamp_time
        Slog_attributes['selected_method'] = selected_method
