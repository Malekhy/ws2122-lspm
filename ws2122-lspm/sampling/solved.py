...
def get_last_activities(logdf):
    ...
    last_activities = log_csv2.drop_duplicates(subset=["case:concept:name"], keep="last")
...
def modifyindex(df):
    ...
    df = df.set_index('case:concept:name')
...
def filterdf(logdf, caseid):
    ...
    return logdf.loc[logdf["case:concept:name"].isin(caseid)]
...
def export_file(df, caseids, s_vci, ci, ac, n=1):
    ...
    export_csv = df[df['case:concept:name'].isin(caseids)]
...
def get_Log_Statistics(log):
    ...
    csss.Parameters.TIMESTAMP_KEY: "time:timestamp"})
