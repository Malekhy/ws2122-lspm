...
log = xes_importer.apply('../eventlogs/repairExample.xes')
...
def test ():
    ...
    computation_sampling(dataframe, ci, ac, unnecessaries, n = 3)
...
def run(request):
    ...
    samppled_file, exported_file = computation_sampling(log_csv, unnecessaries , ci, ac, x)
...
#extract just the very last row of each cases and separate Categorical Columns (ccols)
    ...
    return render(request, 'upload.html',{})
            ...
            #return render(request, 'upload.html',{ 'vars': vars, 'Slog_attributes': Slog_attributes, 'exported_file':exported_file})
